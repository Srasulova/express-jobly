...
describe("GET /companies", function () {
  test("ok for anonymous users", async function () {
    const resp = await request(app).get("/companies");
    expect(resp.body).toEqual({
      companies: [
        {
  ...
  test("fails: invalid filter query", async function () {
    const resp = await request(app)
      .get("/companies")
      .query({ invalidFilter: 2 });
  ...
  test("fails: test next() handler", async function () {
    // This test simulates an error by dropping the companies table
