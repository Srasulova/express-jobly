...
describe("PATCH /users/:username", () => {
  test("works for users: update self", async function () {
    const resp = await request(app)
      .patch(`/users/u1`)
      .send({
        firstName: "New",
  ...
  test("works for admin users: update any user", async function () {
    const resp = await request(app)
      .patch(`/users/u2`)
      .send({
        firstName: "Updated",
  ...
  test("unauth for non-admin users updating other users", async function () {
    const resp = await request(app)
      .patch(`/users/u2`)
      .send({
        firstName: "Updated",
...
describe("DELETE /users/:username", function () {
  ...
  test("works for users deleting self", async function () {
    const resp = await request(app)
      .delete(`/users/u1`)
      .set("authorization", `Bearer ${userToken}`);
    expect(resp.body).toEqual({ deleted: "u1" });
  ...
  test("unauth for non-admin users deleting others", async function () {
    const resp = await request(app)
      .delete(`/users/u2`)
      .set("authorization", `Bearer ${userToken}`);
    expect(resp.statusCode).toEqual(403);
